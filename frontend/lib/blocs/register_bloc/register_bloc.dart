...
class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  ...
  void _onRegisterUser(RegisterUser event, Emitter<RegisterState> emit) async {
    ...
    try {
      if (event.username.isEmpty || event.username.length > maxUsernameLength)
        throw GrpcError.invalidArgument();
      if (event.password.isEmpty || event.password.length > maxPasswordLength)
        throw GrpcError.invalidArgument();
      if (event.passwordCheck.isEmpty || event.passwordCheck != event.password)
        throw GrpcError.invalidArgument();
